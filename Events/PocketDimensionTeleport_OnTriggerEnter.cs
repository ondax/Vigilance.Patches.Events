    ...
    public static class PocketDimensionTeleport_OnTriggerEnter
    {
        public static bool Prefix(PocketDimensionTeleport __instance, Collider other)
        {
            try
            {
                ...
                if (component != null)
                {
                    Player player = Server.PlayerList.GetPlayer(ReferenceHub.GetHub(component.gameObject));
                    if (player == null)
                        return false;
                    if (__instance.type == PocketDimensionTeleport.PDTeleportType.Killer || BlastDoor.OneDoor.isClosed)
                    {
                        player.Hub.playerStats?.HurtPlayer(new PlayerStats.HitInfo(999990f, "WORLD", DamageTypes.Pocket, 0), player.GameObject, true);
                        return false;
                    }
