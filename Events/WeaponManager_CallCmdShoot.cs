    ...
    public static class WeaponManager_CallCmdShoot
    {
        public static bool Prefix(WeaponManager __instance, GameObject target, HitBoxType hitboxType, Vector3 dir, Vector3 sourcePos, Vector3 targetPos)
        {
            try
            {
                ...
                if (!ConfigManager.ShouldKeepScp268)
                ...
                if (target != null)
                    myTarget = Server.PlayerList.GetPlayer(target);
                if (myTarget == null)
                    return true;
                if (myTarget != null && __instance.GetShootPermission(myTarget.Hub.characterClassManager))
                {
                    if (Math.Abs(__instance._hub.playerMovementSync.RealModelPosition.y - myTarget.Hub.playerMovementSync.RealModelPosition.y) > 35f)
                    {
                    ...
                    if (Vector3.Distance(myTarget.Hub.playerMovementSync.RealModelPosition, targetPos) > 5f)
                    {
                    ...
                    if (myTarget.Hub.gameObject == __instance.gameObject)
                    {
                    ...
                    Vector3 vector = myTarget.Hub.playerMovementSync.RealModelPosition - __instance._hub.playerMovementSync.RealModelPosition;
                    if (Vector3.Angle(vector, __instance.transform.forward) > 45f && Math.Abs(vector.y) > 10f && vector.sqrMagnitude < 0.25f)
                    {
                    ...
                    if ((__instance._lastRotation - __instance._hub.playerMovementSync.Rotations).sqrMagnitude < 0.001f)
                    {
                    ...
                    RoleType curClass = myTarget.Role;
                    ...
                    if (curClass != RoleType.Scp173)
                    {
                        ...
                        if (hitboxType > HitBoxType.ARM)
                        {
                            ...
                            {
                                num3 *= 4f;
                                float num4 = 1f / (__instance.weapons[__instance.curWeapon].shotsPerSecond * __instance.weapons[__instance.curWeapon].allEffects.firerateMultiplier);
                ...
                IL_6E6:
                    ...
                    __instance._hub.playerStats.HurtPlayer(new PlayerStats.HitInfo(num3, __instance._hub.LoggedNameFromRefHub(), DamageTypes.FromWeaponId((int)__instance.curWeapon), __instance._hub.queryProcessor.PlayerId), myTarget.GameObject, false);
                    __instance.RpcConfirmShot(true, (int)__instance.curWeapon);
                    __instance.PlaceDecal(true, new Ray(__instance.camera.position, dir), (int)myTarget.Role, num2);
                    return false;
                }
                else
                {
                    BreakableWindow window = target?.GetComponent<BreakableWindow>();
                    if (target != null && hitboxType == HitBoxType.WINDOW && window != null)
                    {
